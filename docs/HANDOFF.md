...
### P1（原 P0，已降级）— 后端英文摘要覆盖率（可选优化）
客户端已适配单摘要设计（见下"已完成的工作"），不再阻塞显示。后端摘要按内容语言**只生成一种**（英文源→英文 `summary_en`、中文源→中文 `summary`），**中文 `summary` 基本为空是设计，无需补齐**。真正可优化的是英文 `summary_en` 覆盖率仅 ~39%（61% 文章无摘要，多因 LLM 429/超时/失败）。完整排查步骤见 [`backend-summary-coverage-checklist.md`](backend-summary-coverage-checklist.md)。这是后端自有优化项，**客户端无需配合**。
